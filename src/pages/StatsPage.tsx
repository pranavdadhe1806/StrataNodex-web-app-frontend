...
function StatCard({ icon, label, value, sub, color }: {
  ...
  return (
    <div style={{
      background: '#22252A', border: '1px solid rgba(255,255,255,0.07)',
      borderRadius: '14px', padding: '20px 24px', flex: '1 1 180px', minWidth: '0',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
        <div style={{ color, display: 'flex' }}>{icon}</div>
        <span style={{ color: '#8A8F98', fontFamily: 'Poppins, sans-serif', fontSize: '12px', fontWeight: 500 }}>
          {label}
        </span>
      </div>
      <div style={{ color: '#D5D8DE', fontFamily: 'Poppins, sans-serif', fontSize: '24px', fontWeight: 700 }}>
        {value}
      </div>
      {sub && <div style={{ color: '#7D828B', fontFamily: 'Poppins, sans-serif', fontSize: '12px', marginTop: '4px' }}>{sub}</div>}
    </div>
  );
}
...
function ProgressBar({ pct, height = 6 }: { pct: number; height?: number }) {
  const color = pct >= 90 ? '#00c896' : pct >= 60 ? '#00bfff' : pct >= 30 ? 'rgba(0,191,255,0.55)' : 'rgba(255,255,255,0.18)';
  return (
    <div style={{ flex: 1, height, background: 'rgba(255,255,255,0.06)', borderRadius: 3, overflow: 'hidden', minWidth: 0 }}>
...
function pctColor(pct: number) {
  return pct >= 90 ? '#00c896' : pct >= 60 ? '#00bfff' : pct >= 30 ? 'rgba(0,191,255,0.7)' : '#8A8F98';
}
...
function ListRow({ list }: { list: ListStat }) {
  return (
    <div style={{
      ...
      <span style={{ color: '#7D828B', fontFamily: 'Poppins, sans-serif', fontSize: '12px', minWidth: '72px', textAlign: 'right' }}>
...
function FolderCard({ folder }: { folder: FolderStat }) {
  ...
  return (
    <div style={{
      ...
      >
        <span style={{ color: '#7D828B', display: 'flex', flexShrink: 0 }}>
          {empty || !open ? <ChevronRight size={15} /> : <ChevronDown size={15} />}
        </span>
        <span style={{ color: '#D5D8DE', fontFamily: 'Poppins, sans-serif', fontSize: '14px', fontWeight: 500 }}>
          📁
        </span>
        <span style={{
          color: '#D5D8DE', fontFamily: 'Poppins, sans-serif', fontSize: '14px',
        ...
        {empty ? (
          ...
          <>
            ...
            <span style={{ color: '#8A8F98', fontFamily: 'Poppins, sans-serif', fontSize: '12px', minWidth: '72px', textAlign: 'right' }}>
    ...
    <div style={{ background: '#1B1D21', minHeight: '100vh' }}>
      ...
      {toast && (
        <div style={{
          ...
          color: '#00bfff', fontFamily: 'Poppins, sans-serif', fontSize: '13px', fontWeight: 500,
        }}>
          {toast}
        </div>
      )}
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '80px 24px 60px' }}>
        ...
        <div style={{
          ...
          padding: '16px 20px',
          background: 'rgba(0,191,255,0.05)', border: '1px solid rgba(0,191,255,0.12)',
          borderRadius: '12px',
        }}>
          <div>
            <div style={{ color: '#D5D8DE', fontFamily: 'Poppins, sans-serif', fontSize: '13px', fontWeight: 500 }}>
              Lock in today's score
            </div>
            <div style={{ color: '#7D828B', fontFamily: 'Poppins, sans-serif', fontSize: '12px', marginTop: '3px' }}>
          ...
          <button
            ...
            style={{
              ...
              background: 'rgba(0,191,255,0.14)', border: '1px solid rgba(0,191,255,0.3)',
        ...
        <div style={{ marginBottom: '20px' }}>
          <h2 style={{ color: '#D5D8DE', fontFamily: 'Poppins, sans-serif', fontSize: '16px', fontWeight: 600, margin: '0 0 6px' }}>
            Current Completion
          </h2>
          <p style={{ color: '#7D828B', fontFamily: 'Poppins, sans-serif', fontSize: '13px', margin: 0 }}>
            Live task completion across all folders and lists — click a folder to expand
          </p>
        </div>

        {summaryLoading ? (
          <div style={{ color: '#7D828B', fontFamily: 'Poppins, sans-serif', fontSize: '13px', padding: '32px 0', textAlign: 'center' }}>
            Loading…
          </div>
        ) : summary.length === 0 ? (
          <div style={{ color: '#7D828B', fontFamily: 'Poppins, sans-serif', fontSize: '13px', padding: '32px 0', textAlign: 'center' }}>
